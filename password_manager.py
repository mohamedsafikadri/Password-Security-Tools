import hashlib 
import getpass 
password_manager ={} 
def create_account(): 
    username = input("Enter your account username: ") 
    password = getpass.getpass("Enter your account password: ") 
    password_hash = hashlib.sha256(password.encode()).hexdigest() 
    password_manager[username] = password_hash 
    print("Account created succefuly!") 
def login(): 
    username = input("Enter your  login username: ") 
    password = getpass.getpass("Enter your login  password: ") 
    password_hash = hashlib.sha256(password.encode()).hexdigest() 
    if username in password_manager.keys() and password_manager[username] == password_hash: 
        print("Login successful!") 
    else: 
        print("invalid username or password") 
def main(): 
    while True: 
        choice =input("Enter 1 to create an account , 2 to login, or 3 to exit: ") 
        if choice =="1": 
            create_account() 
        elif choice == "2": 
            login() 
        elif choice == "3": 
            break 
        else: 
            print("Invalid choice try other number" ) 
if __name__ =="__main__": 
    main()